...
public class AI_Behavior : MonoBehaviour
{
    ...
    [Tooltip("Множитель скорости (для глобального замедления/ускорения). 1.0 = нормальная скорость, 0.5 = в 2 раза медленнее")]
    [Range(0.1f, 2f)]
    ...
    void Start()
    {
        nav.speed = patrolSpeed * speedMultiplier;
        nav.GoToNextPoint();
    }
    ...
    {
        nav.Resume();
        nav.speed = fleeSpeed * speedMultiplier;
